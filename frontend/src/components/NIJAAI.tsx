...
// Determine API URL based on environment variable or default to current IP
const API_URL = process.env.REACT_APP_API_URL || 'http://3.111.22.56:3002/api';
...
useEffect(() => {
  ...
  const wsUrl = process.env.REACT_APP_WS_URL || 'ws://3.111.22.56:3002';
  ...
  try {
    ...
    socket.on('connect', () => {
      ...
      setApiError(null); // Clear any previous error messages

      // Subscribe to contract generation events if options are selected
      if (selectedOptions.entityType && selectedOptions.transactionType && selectedOptions.contractType) {
        socket.emit('subscribe-contract-generation', {
    ...
    // Listen for contract generation progress
    socket.on('contract-generation-progress', (data: { progress: number, message: string }) => {
      console.log('Contract generation progress:', data);
      // Update progress state
      setProgress(data.progress);
      setProgressText(data.message);

      // Clear any error messages when we receive progress updates
      setApiError(null);
    });

    // Connection error with fallback
    socket.on('connect_error', (error) => {
      console.log('WebSocket connection error:', error.message);
      // Don't show error to user, just log it
      console.log('WebSocket connection failed, but application will continue to function');
    });
    ...
    return () => {
      ...
      if (selectedOptions.entityType && selectedOptions.transactionType && selectedOptions.contractType) {
        socket.emit('unsubscribe', {
          event: 'contract',
          id: `${selectedOptions.entityType}:${selectedOptions.transactionType}:${selectedOptions.contractType}`
        });
      }
      socket.disconnect();
    };
    ...
    // Don't show error to user, application will still work without WebSocket
  }
}, [selectedOptions.entityType, selectedOptions.transactionType, selectedOptions.contractType]);
    ...
    try {
      ...
      while (retries >= 0 && !success) {
        try {
          ...
          setProgressText(`Generating your contract... attempt ${2-retries + 1}/3. This may take 1-2 minutes.`);
          ...
          // Log error but don't show it to the user during retries
          console.error('API request failed, retrying...');

          // Update progress text instead of showing an error
          setProgressText(`Generating your contract... attempt ${2-retries + 2}/3. Retrying...`);
    ...
    <Backdrop open={Object.values(loadingStates).some(state => state)} sx={{ zIndex: 9999 }}>
      <Box sx={{ textAlign: 'center', color: 'white' }}>
        ...
        <Typography variant="h6" sx={{ mt: 2 }}>
          {loadingStates.generating && (progressText || 'Generating Contract...')}
        ...
        <Typography variant="body2" sx={{ mt: 1, opacity: 0.8 }}>
          {loadingStates.generating && 'This may take 1-2 minutes with free tier models'}
        </Typography>
        <LinearProgress
  ...
  // State for tracking contract generation progress
          ...
          >
              ...
              Generated Contract
                ...
                <IconButton
                  ...
                  sx={{ color: 'white' }}
                  type="button"
                  aria-label="Copy contract code to clipboard"
                >
                  <ContentCopyIcon />
                </IconButton>
              </Box>
